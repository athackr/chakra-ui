...
`emotion-theming` peer dependencies.

- If you're not using these libraries in your code, you can remove them and
  update to v1.

- If you're using any of `emotion/core` or `emotion/styled`, ensure you pin the
  versions as follows: `"@emotion/core": "10.0.35"`,
  `"@emotion/styled": "10.0.27"`.

You'll need also install `framer-motion`. We use this to manage animations and
transitions within components.

```diff
"dependencies": {
  "@hackr/chakra-ui-core": "latest",
...
Chakra moved all icons to a separate package `@hackr/chakra-ui-icons`. We recommend
using `react-icons` in your projects considering it has a robust set of icons.
However, you can still install this package.

#### Notes on Emotion versions

Chakra UI uses emotion's `ThemeContext` to provide tokens to all components. If
the version of `@emotion/core` in your `node_modules` is different from that
used in Chakra, you may get a strange error or notice that most components lose
their styles. Two ways to fix this are:

- Delete `node_modules` and `package-lock.json` or `yarn.lock` after upgrading.
- Pin the version of `@emotion/core` to `10.0.35` and `@emotion/styled` to
  `10.0.27`.
...
```diff
- import { AccordionHeader } from "@hackr/chakra-ui-core"
+ import { AccordionButton } from "@hackr/chakra-ui-core"
```
...
```diff
- import { AspectRatioBox } from "@hackr/chakra-ui-core"
+ import  { AspectRatio } from "@hackr/chakra-ui-core"
```
...
```diff
import { PhoneIcon } from "@hackr/chakra-ui-icons"
...
[`@hackr/gatsby-plugin-chakra-ui`](https://www.npmjs.com/package/@hackr/gatsby-plugin-chakra-ui)
...
- Basic interface icons provided by Chakra have moved to the `@hackr/chakra-ui-icons`
  package. Replace all `<Icon name="..." />` elements imported from
  `@hackr/chakra-ui-core` with equivalent React elements imported from
  `@hackr/chakra-ui-icons`.

  > Replacement logic: If `<Icon name="search" />` is used, then replace it with
  > `<SearchIcon />` from Chakra icons package.

```diff
- import { Icon } from "@hackr/chakra-ui-core"
- <Icon name="search" />
...
```jsx live=false
import { createIcon } from "@hackr/chakra-ui-core"
...
```jsx live=false
import { createIcon, MoonIcon, SunIcon } from "@hackr/chakra-ui-icons"
```
...
```diff
import { PhoneIcon } from "@hackr/chakra-ui-core"
