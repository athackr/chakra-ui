...
regardless of whether it is controlled or uncontrolled.

## Import

```js
import { useControllableState } from "@hackr/chakra-ui-core"
```
...
Using this hook lets you use the initial state by default with an option to
control the state via control props (using `value`).
