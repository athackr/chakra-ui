---
title: Number Input
package: "@hackr/chakra-ui-number-input"
...
<ComponentLinks
  github={{ url: "#" }}
  storybook={{ url: "#" }}
  npm={{ label: "@hackr/chakra-ui-number-input", url: "#" }}
/>
...
The `NumberInput` component supports the ability to increment or decrement values
using the mouse wheel events. To activate this, pass the `allowMouseWheel` prop.
...
`NumberDecrementStepper` and `NumberIncrementStepper` composes the `Box` props so
you can pass all `Box` props.
