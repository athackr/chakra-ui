...
<ComponentLinks
  github={{ url: "#" }}
  storybook={{ url: "#" }}
  npm={{ label: "@hackr/chakra-ui-icon", url: "#" }}
/>
...
```bash
npm i @hackr/chakra-ui-icons@next

# or

yarn add @hackr/chakra-ui-icons@next
```

### Usage

Import the icons from `@hackr/chakra-ui-icons` and style them with
[style props](/docs/features/style-props).

```jsx live=false
import {PhoneIcon, AddIcon, WarningIcon} from '@hackr/chakra-ui-icons'
...
They can be imported from `@hackr/chakra-ui-core`:

```jsx live=false
import { Icon, createIcon } from "@hackr/chakra-ui-core"
```
...
```jsx live=false
import { createIcon } from "@hackr/chakra-ui-icons"
